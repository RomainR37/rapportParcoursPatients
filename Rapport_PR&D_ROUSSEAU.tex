...
Ajouter un message d'avertissement peut être implémenté facilement sur les différentes vues de l'application. Pour afficher un message de confirmation, il suffit d'ajouter le code suivant: \javacode{onclick="return confirm();"} avec le message désiré sur le lien de l'action que l'on souhaite dans une balise HTML \javacode{<a>}, en l'occurence la suppression dans notre cas. L'ajout du message va concerner les onglets de gestion de ressource (Personnel, Ressources Matérielles, Activités et Parcours) ainsi que l'onglet d'affichage des patients. 
...
La définition des règles du formulaire se fait au sein du contrôleur du patient, \javacode{Patient.php}. Elle se fait simplement en utilisant la méthode \javacode{set\_rules()} prédisposée du framework \textit{CodeIgniter}. Les règles sont ensuite définies en précisant la colonne concernée en premier paramètre telles qu'afficher dans la \autoref{fig:rulesForm}.
...
Les champs numériques sont précisés par le mot-clé \javacode{numeric}, tandis les champs obligatoires sont annotés comme \javacode{required} dans les règles. Pour la gestion de la taille de l'entrée, il est possible d'utiliser \javacode{exact\_length[]} pour donner une taille exacte ou encore \javacode{max\_length[]} pour préciser le nombre maximal de chiffre qui peut figurer pour le champ.
...
Pour résoudre cette erreur, j'ai implémenté une méthode intitulée \javacode{getDisponibiliteRessource()} qui permet de vérifier si la ressource sélectionnée est déjà utilisée ou non pour l'intervalle de temps indiqué. Si celle-ci n'est pas affectée à une autre activité sur cet intervalle, alors on y place l'activité. 
...
Pour corriger ce type d'erreur d'affichage, il suffit de préciser au préalable le parcours concerné dans la méthode récupérant les contraintes de précédences, c'est-à-dire \javacode{getDependancesActivites()} dans la classe \textit{M\_parcours}. 
...
%TODO Afficher le cmd des tests et faire des tests si possible

Les tests se lancent en invite de commande en utilisant le script \javacode{codecept run} en précisant \javacode{--steps} si l'on souhaite voir les étapes de tests. Les détails concernant la mise en place du framework se trouvent sur leur site \url{https://codeception.com/quickstart}.

Les résultats sont affichés comme on peut le voir sur la %TODO AJOUTER L'IMAGE 
...
%TODO Ajouter la notice du dernier livrable
